...
import org.apache.core.e.e.GameRenderListener;
...
public class PH extends M implements PlayerTickListener, AttackEntityListener, GameRenderListener
{

	public PH()
	{
		super("Placement Highlight", "Highlights optimal placements for obsidians", false, C.RENDER);
	}
	...
	public void onPlayerTick() {
		...
		if (placeObiClock == 0)
		{
			...
			if (highlight != null)
			{
				IU.selectItemFromHotbar(Items.OBSIDIAN);
				BU.placeBlock(highlight);
			}
		}
		else
		{
	...
	public void onAttackEntity(AttackEntityEvent event)
	{
		...
		Stream<BlockPos> blocks = BU.getAllInBoxStream(blockPos.add(-4, 0, -4), blockPos.add(4, 0, 4));

		BlockPos placement = blocks
				.filter(b -> !BU.hasBlock(b))
				.filter(b -> BU.hasBlock(b.add(0, -1, 0)))
				.filter(b -> !Box.of(Vec3d.ofCenter(b), 1, 1, 1).intersects(targetBoxAtPlaceObi))
				.filter(b ->
				{
					Vec3d startP = RU.getCameraPos();
					Vec3d endP = Vec3d.ofBottomCenter(b);
					if (endP.subtract(startP).lengthSquared() > 16)
						return false;
					BlockHitResult result = MC.world.raycast(new RaycastContext(RU.getCameraPos(), endP, RaycastContext.ShapeType.OUTLINE, RaycastContext.FluidHandling.NONE, MC.player));
					return result.getType() == HitResult.Type.MISS;
				})
				.filter(b -> CrU.canPlaceCrystalClientAssumeObsidian(b, targetBoxAtPlaceCrystal))
				.max(Comparator.comparingDouble(b -> DU.crystalDamage(target, targetPosAtBreakCrystal, Vec3d.ofCenter(b, 1), b, false)))
				.orElse(null);
		if (placement == null)
			return;
		Ro rotator = Client.INSTANCE.rotator();
		rotator.stepToward(Vec3d.ofBottomCenter(placement), placeObiAfter, () ->
		{
			...
			if (result == ActionResult.SUCCESS)
			{
				...
				rotator.stepToward(new R(0, true, MC.player.getPitch() - 15, false), 5, () ->
				{
	...
	public void onGameRender(MatrixStack matrixStack, float tickDelta)
	{
		...
		matrixStack.push();
		RU.applyRegionalRenderOffset(matrixStack);
		BlockPos blockPos = RU.getCameraBlockPos();
		...
		RU.drawSolidBox(new Box(BlockPos.ORIGIN), matrixStack);
		...
		RU.drawSolidBox(MC.player.getBoundingBox().offset(MC.player.getPos().multiply(-1)), matrixStack);
